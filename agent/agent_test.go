...
func TestNewAgent(t *testing.T) {
	...
	conf.Apps = []AppConf{AppConf{Name: "test", Type: "ruby", Path: "../test/data/"}}
	client := &mocks.Client{}
	//Make sure that we call a heartbeat and register the server
	client.On("HeartBeat").Return(nil).Once()
	gemfile, _ := gemfile.ParseGemfile("../test/data/Gemfile.lock")
	_ = gemfile
	...
	assert.Equal("../test/data/", app.Path, "app path")
	assert.Equal(1, len(app.watchedFiles), "len app.WatchedFiles")

	wf := app.watchedFiles[0]
	assert.Equal("../test/data/Gemfile.lock", wf.GetPath(), "gem file path")
